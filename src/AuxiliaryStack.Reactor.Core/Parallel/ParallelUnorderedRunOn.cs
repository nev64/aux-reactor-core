...
namespace AuxiliaryStack.Reactor.Core.Parallel
{
    sealed class ParallelUnorderedRunOn<T> : ParallelUnorderedFlux<T>
    {
        ...
        internal ParallelUnorderedRunOn(IParallelFlux<T> source, IScheduler scheduler,
            int prefetch)
        {
            this.source = source;
        ...
        internal abstract class BaseParallelObserveOn : ISubscriber<T>, ISubscription
        {
            ...
            internal BaseParallelObserveOn(IWorker worker, int prefetch)
            {
                this.prefetch = prefetch;
        ...
        internal sealed class ParallelObserveOnSubscriber : BaseParallelObserveOn
        {
            ...
            internal ParallelObserveOnSubscriber(ISubscriber<T> actual, int prefetch, IWorker worker)
        ...
        internal sealed class ParallelObserveOnConditionalSubscriber : BaseParallelObserveOn
        {
            ...
            internal ParallelObserveOnConditionalSubscriber(
                IConditionalSubscriber<T> actual, int prefetch, IWorker worker)
                : base(worker, prefetch)
            {
                this.actual = actual;
